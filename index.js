const Keyboard = {
  ...
  init() {
    ...
    document.querySelectorAll(".use-keyboard-input").forEach((element) => {
      element.addEventListener("focus", () => {Keyboard.open(element.value, (currentValue) => {
        ...
        case "backspace":
          ...
          keyElement.addEventListener("click", () => {
            Keyboard.properties.value = Keyboard.properties.value.substring(
              0, Keyboard.properties.value.length - 1);
  ...
  _toggleCapsLock() {
    ...
    for (const key of Keyboard.elements.keys) {
      if (key.childElementCount === 0) {
        key.textContent = Keyboard.properties.capsLock
